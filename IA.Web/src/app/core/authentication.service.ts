...
public refreshToken():Observable<AccessToken>{
    ...
    .switchMap((refresh_token:string) => {
        ...
        return <Observable<AccessToken>>this.http.post(`${environment.api}tokens`,refreshToken);
    }).do((token) =>{
...
private saveAccessData(token: AccessToken) {
    ...
    this.tokenStorage.setCompanyId(data.company_id);
    let roleKey = ''
...
public login(username : string,password : string): Promise<any>{
    ...
    return this.http.post(`${environment.api}tokens`,authData)
    .toPromise()
    ...
    .catch( err =>{
        this.spinnerService.RemoveSpinner();
          this.notificationService.danger("ERROR",err.error)
    })
}
...
public loginWithRefreshToken(refreshTOken : string): Promise<any>{
    ...
    return this.http.post(`${environment.api}tokens`,refreshToken)
    .toPromise()
