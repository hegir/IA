        ...
        for (let errorKey in form.controls[key].errors) {
          switch (errorKey) {
            case 'required':
              ...
              var myStr = this.translateService.instant("REQUIRED_FIELD") as string;
              errors[key].push(myStr);
              break;
            case 'email':
              errors[key].push(this.translateService.instant("WRONG_EMAIL_ADDRESS"));
              break;
            case 'minlength':
